...
if not logger.handlers:
    fh = logging.FileHandler('traj_xai2_log.txt', mode='a', encoding='utf-8')
...
class TrajectoryManipulator2:
    """
    Manipulates trajectories to explain model predictions using XAI techniques.

    (v2 includes temporal data in the process)  
    (works with sliding window segmentation and gaussian perturbation)
    """
    ...
    def _perturbation(self, segment: List[List[float]]) -> List[List[float]]:
        return self.perturbation_model(segment, has_time=self.has_time)
